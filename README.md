...
ISR-інкрементальна статична=підхід найкращий який бере зі сторони SSG  і SSR=буде регенерація сторінок але сторінки будуть побудовані
одразу на етапі (run build) ш інйо буде вниз+х змінюватися, і саме ми будемо визначати коли вона буде змінюватися (час,секунди)

є декілька варіантів сповукувати перезапит в статичній сторінці.
// {next: {revalidate: 5}}- це задоно час на обновлення даних=революдувати сторінку?коли пишемо там 0 то сторінку перетворює 
із статичної на динамічну сторінку.= тобто робиться інший підхід.
{cache:'no-cache'}-динамічна буде сторінка

CSR=клієнсткий компонент,пофакту це реактовський компонент.Навішо в нексті реакт тому що сервер немає доступу до різноманітних
клієнських.Якщо пропишемо 'use client'- то можемо тут використовувати,реакт,юзстейт,юзуфект додавати і створювати хуки.
Можна це використати в Компоненті 'use client' і передати його в page урок 3.4 7.56.
'use client'-можна використовувати усюди де можна де потрібно використовувати хуки клієнські,які доступауться до урли чи с формами
заюзати хук чкий працює з формою